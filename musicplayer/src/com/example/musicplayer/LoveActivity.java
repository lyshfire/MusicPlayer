...
public class LoveActivity  extends Activity{
	 ...
	 private List<FileInfo> mp3InfoLoveList = new ArrayList<FileInfo>();
	...
	private int loveListPosition = -1;   //标识收藏列表位置
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mp3InfoLoveList = helperFavorite.getAllMP3InfoDB(); //获取收藏夹数据库中的mp3列表
    ...
    public   void setListAdpter(List<HashMap<String, String>> mp3list) {  
        if(mp3list != null){
        	mAdapter = new SimpleAdapter(LoveActivity.this, mp3list,  
            ...
            //Toast.makeText(NearActivity.this, "添加列表成功",Toast.LENGTH_SHORT).show();
        }else{
        	Toast.makeText(LoveActivity.this, "本地列表没有歌曲", Toast.LENGTH_SHORT).show();
        }       
    }
  ...
  //给每一个按钮设置监听器
			...
			switch (v.getId()) {  
			     case R.id.return_bt: {// 跳到播放界面
			    	 Bundle toMainData = new Bundle();
			    	    ...
			    	    LoveActivity.this.setResult(0, intentFromMain); 
			    	    LoveActivity.this.finish();
				         break; }
			     case R.id.music_album_iv :  { // 跳到播放界面			          
			         Intent intent = new Intent(LoveActivity.this,PlayingActivity.class);	
			     ...
			     case R.id.music_play_bt:   {//播放音乐
			    	 if(isFirstTime) {                      
		                    ...
		                    Toast.makeText(LoveActivity.this,"开始播放",Toast.LENGTH_SHORT).show(); 
		                } else {  
		                    if (isPlaying) {  		                                            
				                ...
				                play_bt_check = false;
				                anim.pause(); //停止旋转
		                        Toast.makeText(LoveActivity.this,"暂停播放  ",Toast.LENGTH_SHORT).show();  
		                    } else if (isPause) {  
				                ...
				                //animSongName.start();
			                    //animSongSinger.start();
		                        Toast.makeText(LoveActivity.this,"继续播放",Toast.LENGTH_SHORT).show();  
		                    }  
		                }  
    ...
    private class ListItemLongClickListener implements OnItemLongClickListener{
		...
		public boolean onItemLongClick(AdapterView<?> parent, View view, int pos,long id) {  
			loveListPosition = pos;			
			//实例化SelectPicPopupWindow
			menuWindow = new DisableFavoritePopupWindow(LoveActivity.this,R.layout.disable_favorite_dialog,itemsOnClick,RadioGroupOnClick);
			//显示窗口
			menuWindow.showAtLocation(LoveActivity.this.findViewById(R.id.RelativeLayoutLoveActivity), Gravity.CENTER_VERTICAL|Gravity.CENTER_HORIZONTAL, 0, 0); //设置layout在PopupWindow中显示的位置
  ...
  //歌曲收藏与取消收藏
    private void songFavorite(){    	
    	  if(helperFavorite.getMP3Info(fileInfos.get(listPosition))){					
	            ...
	            btn_love.setSelected(false);
	            Toast.makeText(LoveActivity.this,"取消收藏",Toast.LENGTH_SHORT).show();  
         } else  {    	    	  
    	          helperFavorite.addMP3InfoDB(fileInfos.get(listPosition));
    	          fileInfos.get(listPosition).setFavorite(true);	
    	    	  btn_love.setSelected(true);
    	          Toast.makeText(LoveActivity.this,"收藏成功",Toast.LENGTH_SHORT).show();     	    	         		
         }    	  
    }
		...
		public void onClick(View v) {
			...
			case R.id.btn_ok:	{	
				if(!isAllDelete){ //删除本首歌曲
					FileInfo mp3Info = mp3InfoLoveList.get(loveListPosition);		
					helperFavorite.deleteMP3InfoDB(mp3Info);
					for(int i=0;i<fileInfos.size();i++){
				          if(fileInfos.get(i).getTitle().equals(mp3Info.getTitle())){
				        	  fileInfos.get(i).setFavorite(false);	
					...
					if(mp3InfoLoveList.remove(loveListPosition) != null){
						 setListAdpter(MediaUtil.getMusicMaps(mp3InfoLoveList));
					}
				}else{  //清空收藏列表
					for(int i=0;i<fileInfos.size();i++){
				          if(fileInfos.get(i).getFavorite()){
				        	  fileInfos.get(i).setFavorite(false);	
				          }else{
				        	  continue;
				          }
					}
					helperFavorite.deleteALLMP3InfoDB();
					mp3InfoLoveList.clear();
					setListAdpter(MediaUtil.getMusicMaps(mp3InfoLoveList));
				}
				break;}
			default:
    ...
    private OnCheckedChangeListener RadioGroupOnClick = new OnCheckedChangeListener(){
		...
		public void onCheckedChanged(RadioGroup group, int checkedId) {
			...
			if (checkedId == R.id.rb_all) {
				isAllDelete = true;
				menuWindow.getView().setText("取消所有收藏?");
			}else{
				isAllDelete = false;
				menuWindow.getView().setText("取消"+"收藏\"" + mp3InfoLoveList.get(loveListPosition).getTitle() +"\"?" + "\r"
				                                                  + mp3InfoLoveList.get(loveListPosition).getUrl() );
			}
		}  	
    };
