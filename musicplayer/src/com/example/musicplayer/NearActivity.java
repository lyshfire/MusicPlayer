...
public class NearActivity  extends Activity{
	...
	private int nearListPosition = -1;   //标识收藏列表位置
	...
	private MApplication appState;
	 ...
	 private List<FileInfo> mp3InfoList = new ArrayList<FileInfo>(); //最近播放列表
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		mp3InfoList = helper.getAllMP3InfoDB(); //获取数据库中的mp3列表
    ...
    public   void setListAdpter(List<HashMap<String, String>> mp3list) {  
        if(mp3list != null){
        	...
        	Toast.makeText(NearActivity.this, "本地列表没有歌曲", Toast.LENGTH_SHORT).show();
        }       
    }
    //初始化歌曲列表
  ...
  //给每一个按钮设置监听器
			...
			switch (v.getId()) {  
			     ...
			     case R.id.music_play_bt:   {//播放音乐
			    	 if(isFirstTime) {                      
		                    ...
		                    Toast.makeText(NearActivity.this,"开始播放",Toast.LENGTH_SHORT).show(); 
		                } else {  
		                    if (isPlaying) {  		                                            
				                ...
				                play_bt_check = false;
		                        anim.pause();//停止旋转
		                        Toast.makeText(NearActivity.this,"暂停播放  ",Toast.LENGTH_SHORT).show();  
		                    } else if (isPause) {  
				                ...
				                //animSongName.start();
			                    //animSongSinger.start();
		                        Toast.makeText(NearActivity.this,"继续播放",Toast.LENGTH_SHORT).show();  
		                    }  
		                } 
    ...
    private class ListItemLongClickListener implements OnItemLongClickListener{
		...
		public boolean onItemLongClick(AdapterView<?> parent, View view, int pos,long id) {  
			...
			//显示窗口
			menuWindow.showAtLocation(NearActivity.this.findViewById(R.id.RelativeLayoutNearActivity), Gravity.CENTER_VERTICAL|Gravity.CENTER_HORIZONTAL, 0, 0); //设置layout在PopupWindow中显示的位置
  ...
  //歌曲收藏与取消收藏
    private void songFavorite(){    	
    	  if(helperFavorite.getMP3Info(fileInfos.get(listPosition))){					
	            ...
	            btn_love.setSelected(false);
	            Toast.makeText(NearActivity.this,"取消收藏",Toast.LENGTH_SHORT).show();  
         } else  {    	    	  
    	          helperFavorite.addMP3InfoDB(fileInfos.get(listPosition));
    	          fileInfos.get(listPosition).setFavorite(true);	
    	    	  btn_love.setSelected(true);
    	          Toast.makeText(NearActivity.this,"收藏成功",Toast.LENGTH_SHORT).show();     	    	         		
         }    	  
    }
    ...
    private OnCheckedChangeListener RadioGroupOnClick = new OnCheckedChangeListener(){
		...
		public void onCheckedChanged(RadioGroup group, int checkedId) {
			...
			if (checkedId == R.id.rb_all) {
				...
				isAllDelete = false;
				menuWindow.getView().setText("删除本条歌曲播放记录"+"\"" + mp3InfoList.get(nearListPosition).getTitle() +"\"?" + "\r"
